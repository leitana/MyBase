package lx.base.apphall.widgets;
...
public class ShadeTabView extends FrameLayout {
    private final String TAG = ShadeTabView.class.getSimpleName();
    ...
    private int selectTextColor = 0xff00a560;
    ...
    public ShadeTabView(Context context, AttributeSet attrs) {
        ...
        mImageView = (ImageView) findViewById(R.id.iv_my_parse_image);
        mTextView = (TextView) findViewById(R.id.textView);

        TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.ShadeTabView);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            switch (attr) {
                case R.styleable.ShadeTabView_normalDrawableSrc:
                    normalDrawableSrc = typedArray.getDrawable(attr);
                    break;
                case R.styleable.ShadeTabView_selectDrawableSrc:
                    selectDrawableSrc = typedArray.getDrawable(attr);
                    break;
                case R.styleable.ShadeTabView_normalTextColor:
                    normalTextColor = typedArray.getColor(attr, 0xff999999);
                    break;
                case R.styleable.ShadeTabView_selectTextColor:
                    selectTextColor = typedArray.getColor(attr, 0xff00a560);
                    break;
                case R.styleable.ShadeTabView_text:
                    text = typedArray.getString(attr);
                    break;
                case R.styleable.ShadeTabView_textSize:
    ...
    public void setTabView(float alpha) {
        setImageView(alpha);
        setTextView(alpha);
    }
